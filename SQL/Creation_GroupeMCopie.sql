...
DROP DATABASE IF EXISTS lightcycbrsentes;
CREATE DATABASE lightcycbrsentes ;
USE lightcycbrsentes ;
...
IF AGE < AGE_MIN THEN
 INSERT INTO LOGERROR(MESSAGE) VALUES ("ATTENTION, TON AGE DOIT ETRE SUPERIEUR A L AGE MINIMUM REQUIS");
...
IF NBR_EVENT >= 100 THEN/* ON REMARQUE LE CHANGEMENT DE STATUT APRES LA FIN DES INSERTIONS*/
...
DELIMITER $$
CREATE TRIGGER MODIF_STATUT_CONTRIBUTEUR 
    BEFORE INSERT ON EVENEMENTS
    FOR EACH ROW 
BEGIN 
    DECLARE NBR_DE_CONTRIBUTION NUMERIC(6,0);
    SET NBR_DE_CONTRIBUTION = (SELECT COUNT(*) FROM EVENEMENTS WHERE NEW.CREATEUR_ID=EVENEMENTS.CREATEUR_ID);
    
    IF NBR_DE_CONTRIBUTION < 1 THEN 
    	UPDATE UTILISATEUR SET STATUT="Beginner" WHERE U_ID = NEW.CREATEUR_ID;
    ELSEIF NBR_DE_CONTRIBUTION <= 10 AND NBR_DE_CONTRIBUTION > 0 THEN
    	UPDATE UTILISATEUR SET STATUT="Contributeur" WHERE U_ID = NEW.CREATEUR_ID;
    ELSEIF NBR_DE_CONTRIBUTION <= 20 AND NBR_DE_CONTRIBUTION > 10 THEN
    	UPDATE UTILISATEUR SET STATUT="Advanced Contributeur" WHERE U_ID = NEW.CREATEUR_ID;
    ELSEIF NBR_DE_CONTRIBUTION <= 100 AND NBR_DE_CONTRIBUTION > 20 THEN
    	UPDATE UTILISATEUR SET STATUT="Super Contributeur" WHERE U_ID = NEW.CREATEUR_ID;
    ELSEIF NBR_DE_CONTRIBUTION > 100 THEN
    	UPDATE UTILISATEUR SET STATUT="GOD" WHERE U_ID = NEW.CREATEUR_ID;
    END IF;
END $$
DELIMITER ;
...
DETERMINISTIC
BEGIN
	DECLARE MOYEN_AGE_EVENT NUMERIC(4,2);
	
	SELECT AVG(AGEUSER) AS MOYENNE_AGE INTO MOYEN_AGE_EVENT FROM (SELECT AGE(ID_USER) AS AGEUSER ,ID_EVENEMENT FROM INSCRIT WHERE ID_EVENEMENT = EVENEMENT ) AS CALCUL_AGE;
...
INSERT INTO THEME(NOM) VALUES ('CULTURE');
INSERT INTO THEME(NOM) VALUES ('SPORT');


INSERT INTO EVENEMENTS (CREATEUR_ID,TITRE_EVENEMENTS,ADRESSE,LONGITUDE,LATITUDE,DATE_DEBUT,DATE_FIN,NBR_DE_PLACE,AGE_MINIMUM,ID_THEME,IMAGE_URL) VALUES ('RYU','VISITE DE MUSEE','MONTPELLIER',3.8968,43.5984,'2019-11-08 ','2019-11-07  ',5,30,1,'../IMAGES/red-art-relaxation-girl-20967.jpg');
INSERT INTO EVENEMENTS (CREATEUR_ID,TITRE_EVENEMENTS,ADRESSE,LONGITUDE,LATITUDE,DATE_DEBUT,DATE_FIN,NBR_DE_PLACE,AGE_MINIMUM,ID_THEME,IMAGE_URL)VALUES ('GON','VISITE DE MUSEE','PARIS',2.287592, 48.862725 ,'2019-12-24 ','2019-12-24 ',8,10,1,'../IMAGES/red-art-relaxation-girl-20967.jpg');

INSERT INTO EVENEMENTS (CREATEUR_ID,TITRE_EVENEMENTS,ADRESSE,LONGITUDE,LATITUDE,DATE_DEBUT,DATE_FIN,NBR_DE_PLACE,AGE_MINIMUM,ID_THEME,IMAGE_URL) VALUES ('GON','TOURNOI DE FOOT','MONTPELLIER',3.8960,43.5984,'2020-01-01 ','2020-01-02 ',NULL,18,2,'../IMAGES/grass-sport-football-soccer-102448.jpg');

INSERT INTO EVENEMENTS (CREATEUR_ID,TITRE_EVENEMENTS,ADRESSE,LONGITUDE,LATITUDE,DATE_DEBUT,DATE_FIN,NBR_DE_PLACE,AGE_MINIMUM,ID_THEME,IMAGE_URL) VALUES ('RYU','TOURNOI DE FOOT','MONTPELLIER',3.9000,43.5984,'2020-01-01 ','2020-01-02 ',2,18,2,'../IMAGES/grass-sport-football-soccer-102448.jpg');
